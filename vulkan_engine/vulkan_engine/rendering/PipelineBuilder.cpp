...
PipelineBuilder& PipelineBuilder::dynamicState(VkDynamicState state) {
    VK_ASSERT(numDynamicStates_ < VK_MAX_DYNAMIC_STATES);
    dynamicStates_[numDynamicStates_++] = state;
    return *this;
}

PipelineBuilder& PipelineBuilder::primitiveTopology(VkPrimitiveTopology topology) {
    inputAssembly_.topology = topology;
    return *this;
}

PipelineBuilder& PipelineBuilder::rasterizationSamples(VkSampleCountFlagBits samples, float minSampleShading) {
...
PipelineBuilder& PipelineBuilder::frontFace(VkFrontFace mode) {
...
PipelineBuilder& PipelineBuilder::vertexInputState(const VkPipelineVertexInputStateCreateInfo& state) {
    vertexInputState_ = state;
    return *this;
}

PipelineBuilder& PipelineBuilder::viewMask(uint32_t mask) {
    viewMask_ = mask;
    return *this;
}

PipelineBuilder& PipelineBuilder::colorAttachments(const VkPipelineColorBlendAttachmentState* states,
    const VkFormat* formats,
    uint32_t numColorAttachments) {
    VK_ASSERT(states);
    VK_ASSERT(formats);
    VK_ASSERT(numColorAttachments <= VK_UTILS_GET_ARRAY_SIZE(colorBlendAttachmentStates_));
    VK_ASSERT(numColorAttachments <= VK_UTILS_GET_ARRAY_SIZE(colorAttachmentFormats_));
...
PipelineBuilder& PipelineBuilder::depthAttachmentFormat(VkFormat format) {
    depthAttachmentFormat_ = format;
    return *this;
}

PipelineBuilder& PipelineBuilder::stencilAttachmentFormat(VkFormat format) {
    stencilAttachmentFormat_ = format;
    return *this;
}

PipelineBuilder& PipelineBuilder::patchControlPoints(uint32_t numPoints) {
    tessellationState_.patchControlPoints = numPoints;
    return *this;
}

PipelineBuilder& PipelineBuilder::shaderStage(VkPipelineShaderStageCreateInfo stage) {
    if (stage.module != VK_NULL_HANDLE) {
        VK_ASSERT(numShaderStages_ < VK_UTILS_GET_ARRAY_SIZE(shaderStages_));
        shaderStages_[numShaderStages_++] = stage;
    }
    return *this;
}

PipelineBuilder& PipelineBuilder::stencilStateOps(VkStencilFaceFlags faceMask,
    VkStencilOp failOp,
    VkStencilOp passOp,
...
PipelineBuilder& PipelineBuilder::stencilMasks(VkStencilFaceFlags faceMask,
...
VkResult PipelineBuilder::build(VkDevice device,
    ...
    if (!VK_VERIFY(result == VK_SUCCESS)) {
        return result;
    }
